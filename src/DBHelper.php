...
class DBHelper
{
     ...
     * @return array 需要更新的 sql 以及绑定值
     */
    public static function batchUpdateCaseWhen(string $tableName, array $where, array $needUpdateFields): array
    {
        $query = '';
        $bindings = [];

        if (empty($where) || empty($needUpdateFields)) {
            return compact('query', 'bindings');
        }
        ...
        $bindings = array_reduce($building, 'array_merge', []);
     ...
     * 1. 这条语句可以一次性插入多条数据，如果数据已经存在则执行更新操作
     * 2. 可以一次性插入多条数据，适用于批量插入数据的场景
     * 3. 可以避免重复插入数据，提高数据的完整性和准确性
     * 4. 如果数据已经存在，则可以直接更新数据，不需要再进行查询操作，但是可能会增加数据库的负载
     * 5. 只能用于更新存在主键或唯一索引的表
     *
     * @param string $tableName 需要更新的表名称
     ...
     * @return array 需要更新的 sql 以及绑定值
     */
    public static function upsert(string $tableName, array $data, array $columns): array
    {
        $insertValues = [];
        $bindings = [];
